...
BFS3:
	...
	bne $t6, 1, nu_este_host3

	#este host 

	move $t8, $t3
	lw $t7, host1
	beq $t7, $t8, gasit_host
	...
	nu_este_host3:
		...
		for_columns3:
			...
			beq $t4, $t2, BFS3	#verificam daca s-a parcurs toate
...
am_terminat:
	...
	li $t1, 2
	beq $t1, $t9, este_necodificat

	lb $t9, a
	lb $t8, z

	#varianta in care este codificat 
	...
	parcurgere:
		...
		beqz $t3, exit_parcurgere

		sub $t4, $t3, 10

		bge $t4, $t9, este_litera_putem_scadea
		blt $t3, $t9, continua_parcurgere
		bgt $t3, $t8, continua_parcurgere
		li $t4, 0
		li $t5, 10
	
		j litera_nu_putem_scadea_10

	litera_nu_putem_scadea_10:

		bge $t4, $t5, continua_parcurgere		
		sub $t7, $t3, 1		

		blt $t7, $t8, transformare
		j continua_litera_nu_putem_scadea_10
	...
	este_litera_putem_scadea:

		sub $t3, $t3, 10
		sb $t3, mesaj($t2)

		j continua_parcurgere

	continua_litera_nu_putem_scadea_10:
